...
def main():
    """
    使用静态任务数据在 YardEnv 环境中运行最早可用时间任务选择策略进行测试。
    """
    print("--- 开始使用静态任务数据评估最早可用时间任务选择策略 ---")
    ...
    print("开始运行最早可用时间任务选择策略...")
    while not terminated and not truncated:
        action_masks = env.action_masks()
        
        # 选择可用时间最早的任务（也就是累计等待时间最长的任务）
        ...
        if len(valid_actions) == 1 and valid_actions[0] == env.env.max_tasks_in_obs:
            ...
            for action_idx in valid_actions:
                if action_idx < env.env.max_tasks_in_obs:  # 排除等待动作
                    # 获取任务的可用时间
                    task_available_time = current_obs["task_list"][action_idx][1]
                    if task_available_time > 0:  # 确保是有效任务
                        if task_available_time < earliest_available_time:
                            earliest_available_time = task_available_time
                            best_action = action_idx
            
            # 如果找到了最早可用的任务，选择它；否则选择等待
    ...
    # 调用环境的绘图方法来生成并保存轨迹图
    env.env.plot_crane_trajectories(save_path="最早可用时间任务选择轨迹图.png")
    print("轨迹图已保存至 最早可用时间任务选择轨迹图.png")
