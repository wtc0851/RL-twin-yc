...
def run(use_static_data: bool = True, enable_render: bool = False, save_plot: bool = True, output_metrics: bool = True):
    """
    使用静态或随机任务数据在 Yardenv 环境中运行最近任务选择策略。
    - use_static_data=True 时，从 JSON 加载静态任务注入环境。
    - use_static_data=False 时，使用环境随机生成。
    """
    if output_metrics:
        print("--- 开始评估最近任务选择策略 (env)，静态数据: {} ---".format(use_static_data))
    ...
    for _ in range(2000):
        ...
        # 死锁检测逻辑：如果连续多次采取“等待”动作且仍有任务积压，则判定为死锁
        # max_tasks_in_obs 是任务动作的最大索引，也是 WAIT 动作的索引
        # wait_action_idx 已经在上面定义了
        if action == wait_action_idx:
            consecutive_waits += 1
        else:
            consecutive_waits = 0
            
        if consecutive_waits > 50 and len(env.unwrapped.task_queue) > 0:
            if output_metrics:
                print(f"⚠️ 警告: 检测到潜在死锁! 连续 {consecutive_waits} 步等待，且仍有 {len(env.unwrapped.task_queue)} 个任务未完成。")
            # 强制退出循环，避免无效等待
            break
